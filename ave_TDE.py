...
# すでに保存したTD誤差を読み込む
with open("data/raw/No1/ep3_result/TD_error.json", "r") as f:
    step_TD = json.load(f)
...
for params, trials in step_TD.items():
    ...
    for td_list in trials:
        if len(td_list) > 0:
            trial_results.append(sum(td_list)/len(td_list))  # 合計の平均を計算
        else:
            trial_results.append(0.0)
    trial_TD[params] = trial_results

# 保存
with open("data/raw/No7/ep3_result/aveTD_error.json", "w") as f:
